node {
    withCredentials([[$class: 'UsernamePasswordMultiBinding',
        ...
        stage('Pull') {
            try {
                git branch: 'main', url: 'https://github.com/beening01/jenkins.git'
        ...
        stage('Deploy') {
            sshagent(credentials: ['ec2-fastapi-server']) {
                ...
                if (env.BUILD_NUMBER.toInteger() != 1) {
                    sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@3.34.255.155 "sudo docker rm -f docker-fastapi"')
                } else {
                    echo "Initial run (BUILD_NUMBER=${env.BUILD_NUMBER}). Skipping docker container removal."
                }
                // sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@3.34.255.155 "pwd"')
                sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@3.34.255.155 "sudo docker run --name docker-fastapi --env-file .env -e TZ=Asia/Seoul -p 80:8000 -d -t ${DOCKER_USER_ID}/fisa-app:${BUILD_NUMBER}"')
            }
